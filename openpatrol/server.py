    ...
    def do_POST(self):
        ...
        try:
            ...
            if path == "/api/v1/settings":
                ...
                if "max_speed_mps" in body: updated["max_speed_mps"]=max(.05,min(.5,float(body["max_speed_mps"])))
                if "site_timezone" in body: updated["site_timezone"]=str(body["site_timezone"])[:80]
                ...
                temp=self.settings_path.with_suffix(".tmp"); temp.write_text(json.dumps(updated,indent=2)+"\n",encoding="utf-8"); temp.replace(self.settings_path); self.audit.append("settings.update",details={"fields":sorted(body)}); return self._json({**updated,"saved":True})
            return self._error(HTTPStatus.NOT_FOUND, "not_found", "API route not found")
...
def create_server(host="127.0.0.1", port=8765, *, data=None, scenario=None, ingest_token=""):
    ...
    try: settings={**defaults,**json.loads(settings_path.read_text(encoding="utf-8"))}
    except (FileNotFoundError,json.JSONDecodeError,OSError): settings=defaults
