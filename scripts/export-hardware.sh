...
export_assembly() {
  ...
  # CSG preserves the complete parametric assembly for interference review and
  # validates that the source parses. Individual fabrication parts remain real
  # DXF/STL. A combined multi-body STL is optional because it is slow and not a
  ...
  if [[ "${OPENPATROL_EXPORT_ASSEMBLY_STL:-0}" == "1" ]]; then
    local rc=0
    openscad -o "$out/assembly.stl" "${defines[@]}" "$preview" || rc=$?
    if [[ ! -s "$out/assembly.stl" ]]; then
      echo "Warning: optional assembly STL unavailable for $id (OpenSCAD exit $rc)." >&2
  ...
  if [[ "${OPENPATROL_RENDER_PREVIEWS:-0}" == "1" ]]; then
    if ! openscad -o "$out/assembly.png" --imgsize=1600,1000 --viewall \
      ...
      echo "Warning: optional PNG preview unavailable; CSG and fabrication files are valid." >&2
    fi
  fi
}
